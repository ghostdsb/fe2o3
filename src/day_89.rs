pub fn sf<'a>(code: &str, tape: &str) -> String {
  ...
  while needle_pos < tape.len() as i32 && needle_pos >= 0 && cmd_pos < code.len() as i32 {
    match (
      ...
      tape.iter().nth(needle_pos as usize).unwrap(),
    ) {
      ('>', _) => {
        needle_pos += 1;
        cmd_pos += 1;
...
mod tests {
  ...
  fn loops() {
    ...
    assert_eq!(sf("[>*]", &"0".repeat(256)), format!("{}", "0".repeat(256)));
    ...
    //  "Your interpreter should also work properly with nested loops");
    assert_eq!(sf("[[]*>*>*>]", "000"), "000");
    //  "Your interpreter should also work properly with nested loops");
    assert_eq!(sf("*>[[]*>]<*", "100"), "100");
    //  "Your interpreter should also work properly with nested loops");
    assert_eq!(sf("[*>[>*>]>]", "11001"), "01100");
    //  "Your interpreter should also work properly with nested loops");
    assert_eq!(sf("[>[*>*>*>]>]", "10110"), "10101");
    //  "Your interpreter should also work properly with nested loops");
  }
}
