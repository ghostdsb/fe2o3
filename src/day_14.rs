pub mod allergies {
  ...
  pub const ALLERGENS: [Allergen; 8] = [
    Allergen::Eggs,
    Allergen::Peanuts,
    Allergen::Shellfish,
  ...
  impl Allergies {
    ...
    pub fn is_allergic_to(&self, allergen: &Allergen) -> bool {
      let is_allergic = |index: u32| !matches!(1 << index & self.0, 0);

      match ALLERGENS.iter().position(|al| al == allergen) {
        Some(index) => is_allergic(index as u32),
        None => false,
      }
    }
